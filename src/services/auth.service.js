...
//! Register
export const register = async ({ full_name, email, password, role = null }) => {
  const { rows: exists } = await pool.query(
    "SELECT * FROM users WHERE email = $1",
    [email]
  );

  if (exists.length > 0) throw new CustomError("Email is already in use", 401);
  ...
  const { rows: newStaff } = await pool.query(query);
